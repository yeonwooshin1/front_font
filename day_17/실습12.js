...
// [2]
const 가계부목록 = 
        [{ 코드 : 1 , 날짜 : '2025-06-11' , 항목명 : '점심 식사' , 금액 : 1000 }, 
         { 코드 : 2 , 날짜 : '2025-06-12' , 항목명 : '교통비' , 금액 : 1500 }, 
        ]
let 마지막인덱스 = 가계부목록.length-1;
let 코드자동번호 = 가계부목록[ 마지막인덱스 ].코드; //2 // 현재 초기 배열의 마지막객체의 코드번호;

// [3] 기능(함수단위) 구현
// 1. 등록함수 정의 : 실행 : 등록버튼을 눌렀(onClick)을 때

function 등록함수(){    console.log('--- 등록함수 exe ---');    // 2. 함수 onClick 확인
    
    // 3. input에 마크업 객체 3개를 각각 가져오기
    const dateInput = document.querySelector('#dateInput');            console.log('--- dateInput exe ---');
    const contentInput = document.querySelector('#contentInput');      console.log('--- contentInput exe ---');
    const moneyInput = document.querySelector('#moneyInput');          console.log('--- moneyInput exe ---');
...
// 1. 전체 조회 함수 , 실행조건 1. js가 열렸을 때 최초 한 번 2. 등록을 성공했을 때
...
function 전체조회함수(){    console.log('--- 전체조회함수 exe ---');
    ...
    let html = '';
    for (let index =0 ; index < 가계부목록.length ; index++){ //배열내 모든(for) 객체의 정보를 순회한다.
        const obj = 가계부목록[index];  // index 번재 객체(정보) 호출
        html += `<tr>
                    <td> ${ obj.날짜 } </td>
                    <td> ${ obj.항목명 } </td>
                    <td> ${ Number(obj.금액).toLocaleString()} </td>
                </tr>`
    }
